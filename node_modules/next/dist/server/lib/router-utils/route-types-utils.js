...
_export(exports, {
    ...
    writeRouteTypesEntryFile: function() {
        return writeRouteTypesEntryFile;
    },
...
async function writeRouteTypesEntryFile(entryFilePath, actualTypesDir, options) {
    const entryDir = _path.default.dirname(entryFilePath);
    if (!_fs.default.existsSync(entryDir)) {
        await _fs.default.promises.mkdir(entryDir, {
            recursive: true
        });
    }
    // Calculate relative path from entry file to actual types directory
    const relativePath = (0, _normalizepathsep.normalizePathSep)(_path.default.relative(entryDir, actualTypesDir));
    ...
    ;
    await _fs.default.promises.writeFile(entryFilePath, lines.join('\n'));
}
