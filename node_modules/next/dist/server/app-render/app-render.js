 ...
 */ async function generateDynamicFlightRenderResult(req, ctx, requestStore, options) {
    ...
    const { dev = false, onInstrumentationRequestError, setReactDebugChannel, isBuildTimePrerendering = false } = renderOpts;
    ...
    const onError = (0, _createerrorhandler.createReactServerErrorHandler)(dev, isBuildTimePrerendering, workStore.reactServerErrorsByDigest, onFlightDataRenderError);
 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, devFallbackParams) {
    ...
    const { dev = false, onInstrumentationRequestError, setReactDebugChannel, setCacheStatus, isBuildTimePrerendering = false } = renderOpts;
    function onFlightDataRenderError(err, silenceLog) {
        return onInstrumentationRequestError == null ? void 0 : onInstrumentationRequestError(err, req, createErrorContext(ctx, 'react-server-components-payload'), silenceLog);
    }
    const onError = (0, _createerrorhandler.createReactServerErrorHandler)(dev, isBuildTimePrerendering, workStore.reactServerErrorsByDigest, onFlightDataRenderError);
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, createRequestStore, devFallbackParams) {
    ...
    const { basePath, buildManifest, ComponentMod: { createElement, renderToReadableStream: serverRenderToReadableStream }, crossOrigin, dev = false, experimental, isBuildTimePrerendering = false, onInstrumentationRequestError, page, reactMaxHeadersLength, setReactDebugChannel, shouldWaitOnAllReady, subresourceIntegrityManifest, supportsDynamicResponse, cacheComponents } = renderOpts;
    ...
    return (0, _tracer.getTracer)().withSpan(renderSpan, async ()=>{
        ...
        const serverComponentsErrorHandler = (0, _createerrorhandler.createReactServerErrorHandler)(dev, isBuildTimePrerendering, reactServerErrorsByDigest, onHTMLRenderRSCError, renderSpan);
        ...
        const htmlRendererErrorHandler = (0, _createerrorhandler.createHTMLErrorHandler)(dev, isBuildTimePrerendering, reactServerErrorsByDigest, allCapturedErrors, onHTMLRenderSSRError, renderSpan);
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    const { allowEmptyStaticShell = false, basePath, buildManifest, ComponentMod, crossOrigin, dev = false, experimental, isDebugDynamicAccesses, isBuildTimePrerendering = false, onInstrumentationRequestError, page, reactMaxHeadersLength, subresourceIntegrityManifest, cacheComponents } = renderOpts;
    ...
    const serverComponentsErrorHandler = (0, _createerrorhandler.createReactServerErrorHandler)(dev, isBuildTimePrerendering, reactServerErrorsByDigest, onHTMLRenderRSCError);
    function onHTMLRenderSSRError(err) {
        if (reportErrors) {
    ...
    const allCapturedErrors = [];
    const htmlRendererErrorHandler = (0, _createerrorhandler.createHTMLErrorHandler)(dev, isBuildTimePrerendering, reactServerErrorsByDigest, allCapturedErrors, onHTMLRenderSSRError);
