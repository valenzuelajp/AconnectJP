   ...
   */ let MaybeComponent = LayoutOrPage;
    ...
    if (!MaybeComponent) {
        return createSeedData(ctx, createElement(Fragment, {
            key: cacheNodeKey
        }, layerAssets, parallelRouteProps.children), parallelRouteCacheNodeSeedData, loadingData, isPossiblyPartialResponse, hasRuntimePrefetch, // No user-provided component, so no params will be accessed. Use the
    ...
    if (workStore.isStaticGeneration && workStore.forceDynamic && experimental.isRoutePPREnabled) {
        ...
        }), layerAssets), parallelRouteCacheNodeSeedData, loadingData, true, hasRuntimePrefetch, // force-dynamic postpones without rendering the component, so no params
    ...
    if (isPage) {
        ...
        if (isClientComponent) {
            ...
            const params = createServerParamsForServerSegment(currentParams, workStore, varyParamsAccumulator);
            // If we are passing searchParams to a server component Page we need to
            // track their usage in case the current render mode tracks dynamic API
            // usage.
            let searchParams = createServerSearchParamsForServerPage(query, workStore, varyParamsAccumulator);
        ...
        return createSeedData(ctx, createElement(Fragment, {
            key: cacheNodeKey
        }, wrappedPageElement, layerAssets, MetadataOutlet ? createElement(MetadataOutlet, null) : null), parallelRouteCacheNodeSeedData, loadingData, isPossiblyPartialResponse, hasRuntimePrefetch, varyParamsAccumulator);
    } else {
        ...
        return createSeedData(ctx, wrappedSegmentNode, parallelRouteCacheNodeSeedData, loadingData, isPossiblyPartialResponse, hasRuntimePrefetch, varyParamsAccumulator);
    }
}
...
function createSeedData(ctx, rsc, parallelRoutes, loading, isPossiblyPartialResponse, hasRuntimePrefetch, varyParamsAccumulator) {
    if (loading !== null) {
